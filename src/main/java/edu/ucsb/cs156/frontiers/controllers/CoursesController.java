...
public class CoursesController extends ApiController {
  ...
  public InstructorCourseView postCourse(
    ...
    if (canvasApiToken != null && !canvasApiToken.isEmpty()) {
      course.setCanvasTokenLastFour(canvasApiToken.substring(canvasApiToken.length() - 4));
      course.setCanvasApiToken(tokenEncryptionService.encryptToken(canvasApiToken));
      course.setCanvasTokenLastUpdated(LocalDateTime.now());
    }
  ...
  /**
   * Returns the Canvas course ID, last four digits of the Canvas token, and the last time the token
   * was updated for a course by its id. The full token is never returned.
   *
  ...
  public Map<String, String> getCourseCanvasInfo(
    ...
    return Map.of(
        ...
        "canvasTokenLastFour",
            course.getCanvasTokenLastFour() != null ? course.getCanvasTokenLastFour() : "",
        "canvasTokenLastUpdated",
            course.getCanvasTokenLastUpdated() != null
                ? course.getCanvasTokenLastUpdated().toString()
                : "");
  }
