...
public class TeamsController extends ApiController {
    ...
    public record TeamMemberResult(TeamMember teamMember, TeamMemberStatus status, String rejectedEmail) {
    ...
    public record TeamCreationResponse(TeamSourceType typeMatched, Integer created, Integer existing, List<String> rejected) {}
    ...
    public Team postTeam(
        ...
        if(teamRepository.findByCourseIdAndName(course.getId(), name).isPresent()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Team with name %s already exists".formatted(name));
        }else{
    ...
    /**
     * Upload teams in CSV format (team, email)
    ...
    public TeamCreationResponse uploadTeamsCsv(
        ...
        try (InputStream inputStream = new BufferedInputStream(file.getInputStream());
            ...
            return new TeamCreationResponse(sourceType, counts[TeamMemberStatus.CREATED.ordinal()], counts[TeamMemberStatus.EXISTS.ordinal()], failed);
        }
    }
    ...
    public TeamMember addTeamMember(
        ...
        if (teamMemberRepository.findByTeamAndRosterStudent(team, rosterStudent).isPresent()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Team member already exists for team %s and roster student %s".formatted(team.getName(), rosterStudent.getEmail()));
        }
    ...
    public TeamMemberResult teamMemberFromSimpleCsv(String[] row, Course course) {
        ...
        if (student.isPresent() && team.isPresent()) {
            ...
            TeamMember saveTeamMember = TeamMember.builder()
                    .team(teamToSave)
                    .rosterStudent(student.get())
                    .build();
