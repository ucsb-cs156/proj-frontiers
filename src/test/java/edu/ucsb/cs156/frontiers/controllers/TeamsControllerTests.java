...
public class TeamsControllerTests extends ControllerTestCase {
    ...
    public void testPostTeam_not_unique() throws Exception {
        ...
        when(courseRepository.findById(eq(1L))).thenReturn(Optional.of(course));
        when(teamRepository.findByCourseIdAndName(eq(1L), eq("Team Beta"))).thenReturn(Optional.of(team));
        ...
        assertEquals("Team with name Team Beta already exists", response.getResponse().getErrorMessage());
    }
    ...
    public void testAddTeamMember_alreadyexists() throws Exception {
        ...
        when(teamRepository.findById(eq(1L))).thenReturn(Optional.of(team));
        when(rosterStudentRepository.findById(eq(1L))).thenReturn(Optional.of(rosterStudent));
        when(teamMemberRepository.findByTeamAndRosterStudent(eq(team), eq(rosterStudent))).thenReturn(Optional.of(teamMember));
        ...
        // assert
        verify(teamMemberRepository, never()).save(any(TeamMember.class));
        assertEquals("Team member already exists for team Team Alpha and roster student student@ucsb.edu", response.getResponse().getErrorMessage());
    }
    ...
    public void testUploadTeamsCsv_success() throws Exception {
        // arrange
        Course course = Course.builder().id(1L).courseName("CS156").build();
        RosterStudent student1 = RosterStudent.builder().email("fakestudent1@ucsb.edu").course(course).build();
        RosterStudent student2 = RosterStudent.builder().email("fakestudent2@ucsb.edu").course(course).build();
        RosterStudent student3 = RosterStudent.builder().email("existingstudent@ucsb.edu").course(course).build();
        ...
        TeamMember teamMemberCreated1 = TeamMember.builder().team(team1).rosterStudent(student1).build();
        ...
        when(courseRepository.findById(eq(1L))).thenReturn(Optional.of(course));
        when(rosterStudentRepository.findByCourseIdAndEmail(eq(1L), eq("fakestudent1@ucsb.edu"))).thenReturn(Optional.of(student1));
        when(rosterStudentRepository.findByCourseIdAndEmail(eq(1L), eq("fakestudent2@ucsb.edu"))).thenReturn(Optional.of(student2));
        when(rosterStudentRepository.findByCourseIdAndEmail(eq(1L), eq("existingstudent@ucsb.edu"))).thenReturn(Optional.of(student3));
        when(teamRepository.findByCourseIdAndName(eq(1L), eq("team1"))).thenReturn(Optional.of(team1));
        when(teamMemberRepository.findByTeamAndRosterStudent(eq(team1), eq(student3))).thenReturn(Optional.ofNullable(teamMemberFor3));
        ...
        TeamsController.TeamCreationResponse expectedResponse = new TeamsController.TeamCreationResponse(TeamsController.TeamSourceType.SIMPLE,2, 1, List.of("nors@ucsb.edu"));
