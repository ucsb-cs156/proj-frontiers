...
public class OrganizationMemberServiceTests {
  ...
  void setUp() throws Exception {
    testCourse = Course.builder().orgName(TEST_ORG).installationId("123").build();
  ...
  void testGetOrganizationMembers_SinglePage() throws Exception {
    List<OrgMember> expectedMembers =
        List.of(
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/members?role=member"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andExpect(header("Accept", "application/vnd.github+json"))
        .andExpect(header("X-GitHub-Api-Version", "2022-11-28"))
        .andRespond(
            withStatus(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(jsonResponse));
  ...
  void testGetOrganizationMembers_MultiplePages() throws Exception {
    ...
    List<OrgMember> firstPageMembers = List.of(orgMember1);
    List<OrgMember> secondPageMembers = List.of(orgMember2);
    ...
    secondPageHeaders.add(
        "link", "<https://api.github.com/orgs/" + TEST_ORG + "/members?page=1>; rel=\"previous\"");
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/members?role=member"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
    ...
    mockServer
        .expect(
            requestTo("https://api.github.com/orgs/" + TEST_ORG + "/members?page=2&role=member"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andRespond(
            withStatus(HttpStatus.OK)
    ...
    List<OrgMember> result =
        (List<OrgMember>) organizationMemberService.getOrganizationMembers(testCourse);
  ...
  void testGetOrganizationAdmins_SinglePage() throws Exception {
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/members?role=admin"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andExpect(header("Accept", "application/vnd.github+json"))
        .andExpect(header("X-GitHub-Api-Version", "2022-11-28"))
  ...
  void testGetOrganizationAdmins_MultiplePages() throws Exception {
    ...
    List<OrgMember> firstPageAdmins = List.of(orgAdmin1);
    List<OrgMember> secondPageAdmins = List.of(orgAdmin2);
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/members?page=2&role=admin"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andRespond(
            withStatus(HttpStatus.OK)
    ...
    List<OrgMember> result =
        (List<OrgMember>) organizationMemberService.getOrganizationAdmins(testCourse);
  ...
  void testInviteOrganizationMember_Success() throws Exception {
    RosterStudent testStudent = RosterStudent.builder().githubId(12345).course(testCourse).build();
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/invitations"))
  ...
  void testInviteOrganizationMember_failure_is_member() throws Exception {
    RosterStudent testStudent =
        RosterStudent.builder().githubId(12345).githubLogin("banana").course(testCourse).build();
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/memberships/" + "banana"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andExpect(header("Accept", "application/vnd.github+json"))
        .andExpect(header("X-GitHub-Api-Version", "2022-11-28"))
  ...
  void testInviteOrganizationOwner_Success() throws Exception {
    CourseStaff testStaff = CourseStaff.builder().githubId(12345).course(testCourse).build();
  ...
  void testGetOrganizationInvitees_SinglePage() throws Exception {
    List<OrgMember> expectedInvitees =
        List.of(
  ...
  void testGetOrganizationInvitees_MultiplePages() throws Exception {
    ...
    List<OrgMember> firstPageInvitees = List.of(orgInvitee1);
    List<OrgMember> secondPageInvitees = List.of(orgInvitee2);
    ...
    HttpHeaders firstPageHeaders = new HttpHeaders();
    firstPageHeaders.add(
        "link", "<https://api.github.com/orgs/" + TEST_ORG + "/invitations?page=2>; rel=\"next\"");

    HttpHeaders secondPageHeaders = new HttpHeaders();
    secondPageHeaders.add(
        "link",
        "<https://api.github.com/orgs/" + TEST_ORG + "/invitations?page=1>; rel=\"previous\"");
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/invitations"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
    ...
    mockServer
        .expect(requestTo("https://api.github.com/orgs/" + TEST_ORG + "/invitations?page=2"))
        .andExpect(method(HttpMethod.GET))
        .andExpect(header("Authorization", "Bearer " + TEST_TOKEN))
        .andRespond(
            withStatus(HttpStatus.OK)
    ...
    List<OrgMember> result =
        (List<OrgMember>) organizationMemberService.getOrganizationInvitees(testCourse);
  ...
  void testRemoveOrganizationMember_Success() throws Exception {
    RosterStudent testStudent =
        RosterStudent.builder().githubId(12345).githubLogin("testuser").course(testCourse).build();
  ...
  void testRemoveOrganizationMember_NullGithubLogin() throws Exception {
    ...
    assertEquals(
        "Cannot remove student from organization: GitHub login is null", exception.getMessage());
    mockServer.verify();
  }
